...
// CUDA Kernel function for vector addition using stride loop
__global__ void vectorAdd(const int *A, const int *B, int *C, int numElements) {
    int i = blockIdx.x * blockDim.x + threadIdx.x; // Compute initial global index
...
int main(int argc, char **argv) {
    ...
    int grid_dim = numSMs * 8 * 256/blk_dim; // Use 8 blocks per SM when 256 threads per block
